...
export default function CreatorProofOfWork() {
  const { user } = useAuth();
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setDialogOpen(false);
      setFormData({ proofUrl: '', proofDescription: '', proofType: 'link' });
      refetch();
    } catch (error: any) {
  ...
  const rejectedPow = proofOfWork?.filter(p => p.status === 'rejected') || [];
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">Submit and track your work submissions</p>
          </div>
          <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>
                  Submit a link or upload proof of your completed work
                </DialogDescription>
              </DialogHeader>
              <div className="space-y-4 mt-4">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    placeholder="https://twitter.com/yourpost or link to your work"
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <div key={pow.id} className="flex items-center justify-between p-3 rounded-lg bg-muted/50">
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        {getStatusIcon(pow.status)}
                        <p className="font-medium text-sm">
                          {pow.tasks?.title || 'General Submission'}
                        </p>
                      </div>
                      <p className="text-xs text-muted-foreground mt-1">
                        {pow.proof_description?.slice(0, 100) || 'No description'}
                      </p>
                    </div>
                    <a 
                      href={pow.proof_url} 
        ...
        {rejectedPow.length > 0 && (
          <Card className="border-border/50">
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <XCircle className="h-5 w-5 text-red-500" />
                Rejected ({rejectedPow.length})
              </CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-3">
                {rejectedPow.map((pow: any) => (
                  <div key={pow.id} className="flex items-center justify-between p-3 rounded-lg bg-muted/50">
                    <div>
                      <div className="flex items-center gap-2">
                        {getStatusIcon(pow.status)}
